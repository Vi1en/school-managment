...
const EditStudent: React.FC = () => {
  ...
  const { students, updateStudent, fetchStudentById } = useStudentsStore();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const result = await updateStudent(admissionNumber!, studentData);
      
      if (result.success) {
        navigate('/students');
      } else {
        setLocalError(result.error || 'Failed to update student');
      }
    } catch (error: any) {
      setLocalError(error.message || 'Failed to update student');
    } finally {
      setLoading(false);
    }
  };
